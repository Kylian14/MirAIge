...
| **Detection cascade (T0/T1/T2)** | The tiered classifier, cheapest first: **T0** Sigma rules, **T1** calibrated heuristics, **T2** an optional LLM. |
...
| **Honeycredential graph** | Fake credentials that branch ≥ 2. Following one leads to more, and never to anything real. |
...
| **BFF / console** | The `api` service: a Backend-For-Frontend that serves the SPA and the authenticated `/api/v1`. The single public surface. |
